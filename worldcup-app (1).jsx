  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: BG }}>
      <div style={{ textAlign: "center", padding: "20px 22px 0" }}>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: DARK, margin: 0 }}>World Cup 2026</h1>
        <p style={{ fontSize: 13, color: "#999", margin: "3px 0 0" }}>USA, Canada & Mexico</p>
      </div>
      <div style={{ flex: 1, position: "relative", overflow: "hidden", margin: "16px 0 0", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ position: "absolute", width: 300, height: 300, borderRadius: "50%", background: BG, boxShadow: SHADOW_IN }} />
        <svg viewBox="0 0 200 280" width="220" height="280" style={{ position: "relative", zIndex: 1, filter: "drop-shadow(0 10px 24px rgba(0,0,0,0.18))" }}>
          <defs>
          ...
          {/* Base bottom */}
          <rect x="55" y="248" width="90" height="16" rx="4" fill="url(#base)"/>
          <rect x="60" y="240" width="80" height="12" rx="3" fill="url(#base)"/>
          {/* Stem */}
          <rect x="85" y="200" width="30" height="44" rx="4" fill="url(#gold2)"/>
          {/* Middle ring */}
          <ellipse cx="100" cy="200" rx="32" ry="8" fill="url(#gold1)"/>
          {/* Cup body */}
          <path d="M60 80 Q55 140 70 180 Q85 200 100 200 Q115 200 130 180 Q145 140 140 80 Z" fill="url(#gold1)"/>
          {/* Cup highlight */}
          <path d="M75 90 Q72 130 80 165 Q88 185 100 190" stroke="rgba(255,255,200,0.5)" strokeWidth="4" fill="none" strokeLinecap="round"/>
          {/* Left handle */}
          <path d="M62 95 Q30 95 28 120 Q28 145 62 148" stroke="url(#gold2)" strokeWidth="10" fill="none" strokeLinecap="round"/>
          <path d="M62 95 Q32 95 30 120 Q30 145 62 148" stroke="rgba(255,220,80,0.4)" strokeWidth="3" fill="none" strokeLinecap="round"/>
          {/* Right handle */}
          <path d="M138 95 Q170 95 172 120 Q172 145 138 148" stroke="url(#gold2)" strokeWidth="10" fill="none" strokeLinecap="round"/>
          <path d="M138 95 Q168 95 170 120 Q170 145 138 148" stroke="rgba(255,220,80,0.4)" strokeWidth="3" fill="none" strokeLinecap="round"/>
          {/* Top opening */}
          <ellipse cx="100" cy="82" rx="40" ry="10" fill="url(#gold2)"/>
          {/* Globe on top */}
          <circle cx="100" cy="58" r="28" fill="url(#gold1)"/>
          <circle cx="100" cy="58" r="28" fill="none" stroke="rgba(160,112,16,0.6)" strokeWidth="1.5"/>
          {/* Globe lines */}
          <ellipse cx="100" cy="58" rx="28" ry="12" fill="none" stroke="rgba(160,112,16,0.5)" strokeWidth="1"/>
          <line x1="100" y1="30" x2="100" y2="86" stroke="rgba(160,112,16,0.5)" strokeWidth="1"/>
          <line x1="72" y1="58" x2="128" y2="58" stroke="rgba(160,112,16,0.5)" strokeWidth="1"/>
          {/* Globe shine */}
          <circle cx="90" cy="48" r="8" fill="rgba(255,245,180,0.35)"/>
          {/* Stars */}
          <text x="100" y="230" textAnchor="middle" fontSize="11" fill="#C8960A">★ ★ ★</text>
        </svg>
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 60, background: "linear-gradient(to bottom, transparent, #EBEBEB)" }} />
      </div>
      <div style={{ margin: "0 20px 14px", background: BG, borderRadius: 20, boxShadow: SHADOW_OUT, padding: "16px 8px 12px", display: "flex" }}>
        {[{ v: d, l: "DAYS" }, { v: h, l: "HOURS" }, { v: m, l: "MINUTES" }, { v: s, l: "SECONDS" }].map(({ v, l }, i) => (
          <div key={l} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", position: "relative" }}>
            <span style={{ fontSize: 30, fontWeight: 700, color: DARK, lineHeight: 1 }}>{String(v).padStart(2, "0")}</span>
            {i < 3 && <span style={{ position: "absolute", right: -2, top: 2, fontSize: 22, fontWeight: 700, color: "#bbb" }}>:</span>}
            <span style={{ fontSize: 8, color: "#aaa", textTransform: "uppercase", letterSpacing: 1, marginTop: 5 }}>{l}</span>
          </div>
        ))}
      </div>
      <div style={{ padding: "0 20px 28px" }}>
        <button onClick={onNext} style={{ width: "100%", background: DARK_BTN, color: "#fff", border: "none", borderRadius: 16, padding: "16px 0", fontSize: 16, fontWeight: 700, cursor: "pointer", boxShadow: SHADOW_BTN_DARK }}>
...
// ── HOME ─────────────────────────────────────────────────────────────────────
function HomeScreen({ onGlobalGroup, onGroupsList, onMyGroups, onBracket }) {
  const leaders = [
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: BG, overflowY: "auto" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px 20px 14px" }}>
        <div>
          <p style={{ fontSize: 13, color: "#aaa", margin: 0 }}>Good afternoon 👋</p>
          <h2 style={{ fontSize: 21, fontWeight: 700, color: DARK, margin: "4px 0 0" }}>My Dashboard</h2>
        </div>
      ...
      <p style={{ fontSize: 14, fontWeight: 700, color: DARK, margin: "0 20px 8px" }}>Top Players</p>
      ...
      {/* Groups section */}
      <p style={{ fontSize: 14, fontWeight: 700, color: DARK, margin: "0 20px 8px" }}>Groups</p>
      <div style={{ padding: "0 20px" }}>
        ...
        <button onClick={onGlobalGroup} style={{ width: "100%", background: DARK_BTN, color: "#fff", border: "none", borderRadius: 14, padding: "15px 20px", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: SHADOW_BTN_DARK, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ textAlign: "left" }}>
            <div>🌍 Global Group</div>
            <div style={{ fontSize: 11, fontWeight: 400, opacity: 0.6, marginTop: 2 }}>48,291 members · Free · Prizes included</div>
          </div>
        ...
        <button onClick={onGroupsList} style={{ width: "100%", background: BG, color: DARK, border: "none", borderRadius: 14, padding: "15px 20px", fontSize: 15, fontWeight: 700, cursor: "pointer", boxShadow: SHADOW_OUT, display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          <div style={{ textAlign: "left" }}>
            ...
            <div style={{ fontSize: 11, fontWeight: 400, color: "#aaa", marginTop: 2 }}>Join a private group or create your own</div>
          </div>
          <span style={{ color: "#bbb", fontSize: 20 }}>›</span>
        </button>

        {/* My Groups & Bracket */}
        <NeuBtn label="📋 My Groups" dark={false} onClick={onMyGroups} />
        <NeuBtn label="🏆 Bracket & Live Scores" dark={false} onClick={onBracket} />
      </div>
      <div style={{ height: 20 }} />
    </div>
  );
    ...
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", minHeight: "100vh", background: "#D4D4D4", fontFamily: "-apple-system, 'SF Pro Display', sans-serif" }}>
      <div style={{ width: 390, height: 844, background: BG, borderRadius: 46, overflow: "hidden", boxShadow: "0 30px 80px rgba(0,0,0,0.22), inset 0 1px 0 rgba(255,255,255,0.8)", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
          {screen === SCREENS.SPLASH && <SplashScreen onNext={() => setScreen(SCREENS.LOGIN)} />}
