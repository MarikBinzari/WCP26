-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Fix score_bracket_predictions: r32_map corect (match_key based)
-- ─────────────────────────────────────────────────────────────────────────────
...
language sql security definer as $$
  ...
  -- Rezultate KO (include match_key pentru R32)
  ko_results as (
  ...
  -- R32: match_key based (sloturile din DB nu corespund maparii vechi)
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Trigger: după orice meci FT → apply_bracket_scores + apply_exact_scores
-- ─────────────────────────────────────────────────────────────────────────────
create or replace function public._trg_match_ft_scoring()
returns trigger language plpgsql security definer as $$
begin
  -- Procesăm doar tranziția NS/LIVE → FT
  if NEW.status != 'FT' or OLD.status = 'FT' then
    return NEW;
  end if;

  perform public.apply_bracket_scores();
  perform public.apply_exact_scores();

  return NEW;
end;
$$;

drop trigger if exists trg_match_ft_scoring on public.live_scores;
create trigger trg_match_ft_scoring
  after update of status
  on public.live_scores
  for each row
  execute procedure public._trg_match_ft_scoring();

-- Aplică retroactiv scorurile corecte pentru R32
