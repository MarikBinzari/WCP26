-- Fix: score group predictions only for groups where ALL matches are FT
-- Previously, partial standings (e.g. 2/6 matches done) were used to award points prematurely
...
language sql security definer as $$
  ...
  -- Grupele unde TOATE meciurile sunt FT
  -- LEFT JOIN ca să numărăm și meciurile fără row în live_scores (viitoare sau nereportate)
  completed_groups as (
  ...
  -- Rezultate meciuri de grupă (doar din grupe complete)
  match_results as (
    select
      m.group_id,
  ...
  -- Scoring grupă (doar grupe complete)
  group_pts as (
